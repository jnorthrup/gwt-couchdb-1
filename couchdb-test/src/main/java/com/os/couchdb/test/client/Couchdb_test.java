package com.os.couchdb.test.client;
 ...
 */
public class Couchdb_test implements EntryPoint {
				...
				if (files != null && files.length == 1) {
					m_db.getDocument(new BaseModelFactory<BaseDocument>() {
						...
						public void onSuccess(BaseDocument pResult) {
							...
							m_db.createOrUpdateAttachment("test", rev, getMainFileName(files[0].getName()), mimeType, files[0]
									.getBlob(), new AsyncCallback<String>() {
								@Override
								public void onFailure(Throwable pCaught) {
									m_dlgBoxResult.setText("Error:" + pCaught);
								}
